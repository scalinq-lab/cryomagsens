...
"""
Diode voltage logging script with the SPU.

- Asks you to insert sensor ID and test type
- Measures the diode voltage once every measurement interval time
...
print("-" * 35)
print(f"{'Time':<10} | {'Diode Voltage':<20}")
print("-" * 35)
...
try:
    with open(filename_log, "w", encoding="utf-8") as log:
        ...
        # Write CSV Header
        log.write("Time,Diode_Voltage\n")

        while True:
            # Read doide voltage from the SPU
            try:
                diode_voltage = SPU.get_latest_data()[1][1]
            except Exception as e:
            ...
            log.write(f"{now_str},{diode_voltage}\n")
            log.flush()

            plot_times.append(now_obj)
            plot_diode_v.append(diode_voltage)

            print(f"{now_str:<10} | {diode_voltage:<20.5f}")
